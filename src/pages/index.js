...
export async function getStaticProps() {
  ...
  const [featured] = await client.fetch(
    `*[_type == "featured_posts" && title == "Homepage"] { posts[]->{_id, title, author, body, badge, date, featuredImage, category, date, type, slug, stackbit_model_type, intro, seo} }`
  );
  ...
  return {
    props: { path: "/", page, data: { config, topics }, featured: featured.posts.slice(0, 4), articles, fellows },
    revalidate: 60,
  };
}
