...
import { fellowships } from "../layouts";
...
export async function getStaticProps(props) {
  ...
  let [page] = await client.fetch(
    `*[!(_id in path("drafts.**")) && _type in ["advanced", "page", "post"] && (slug.current == "${slug}" || stackbit_url_path == "/${slug}")]{_id, _type, img_path, heroImage, stackbit_url_path, trackerText, _createdAt, _updatedAt, badge, date, slug, title, description, faq, body, toc, tocTitle, featuredImage, seo, disableNewsletterSignup, authors[]->{slug, name, photo, bio}, heroContent, layout, sections, sidebar_content[type == "sidebar_about"]{staff[]->, board[]->, masthead[]->}, relatedTopics[]->{_id, displayName, name, topicType, slug, stackbit_model_type}, relatedArticles[]->{date,badge,title,slug,authors[]->{firstName, lastName}},relatedCommentary[]->}`
  ); 
